...
def solve_row(xs, ws, yd, alpha, theta):
    ya = get_output(xs, ws, theta)
    e = yd - ya
...
def full_solve(ixs, ws, yds, alpha, theta, epoch=5):
    for i in range(1, epoch + 1):
        print(f"Epoch {i}".center(15, "="))
        r = cal_epoch(ixs, ws, yds, alpha, theta)
        ws = r["ws"]
...
full_solve(ixs, ws, yd, alpha, theta, epoch=6) 
